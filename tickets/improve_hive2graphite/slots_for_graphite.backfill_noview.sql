...
WITH 
    ...
    SELECT
        ...
        memory/2.5/1000 as slot_hours,
        job_id
    FROM
         ${hiveconf:DB_NAME}.container_time_series container_time_series
    ...
    WHERE
        system LIKE '%${hiveconf:SYSTEM}%' AND
        state != 'RESERVED' AND
        date >= date_sub('${hiveconf:START_DATE}', ${hiveconf:MAX_LOOKBACK})
    )
    ...
    LEFT JOIN 
        ...
        (slots_with_hour_and_day.system = ${hiveconf:JOB_TABLE}.system) AND (slots_with_hour_and_day.job_id = ${hiveconf:JOB_TABLE}.jobid)
    WHERE
    	${hiveconf:JOB_TABLE}.date >= date_sub('${hiveconf:START_DATE}', ${hiveconf:MAX_LOOKBACK})
    )
